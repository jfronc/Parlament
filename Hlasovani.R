...
zarazeni_kluby <- zarazeni %>% filter(of_id %in% kluby$org_id)

zarazeni_kluby %>% filter(since <= as.Date("2023-01-01") & (until >= as.Date("2022-01-01") | is.na(until))) %>% group_by(of_id) %>% summarize(count = n())
